...
Because this gene model annotation dataset contains many alternatively spliced genes, some genes are listed multiple times. As a result, the `annotations` data frame has many 
duplicate rows, rows that will mess up the next steps. 
...
The gene model annotations data frame now contains 
`r format(nrow(annotations),big.mark=",")` rows representing different SL5 gene
model annotations.
